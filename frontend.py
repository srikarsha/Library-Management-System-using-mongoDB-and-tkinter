...
def login_window(round):
    ...
    # User ID Label
    ID = Label(myFrame, text="Username:", relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12), padx=20,
               pady=20)
    ...
    # Login Button
    loginButton = Button(myFrame, image=round, borderwidth=0, bg="#3b404e", fg="white",
                         command=lambda: value(idEnter, passEnter))
    ...
    register = Button(myFrame, text="Create new account", fg="white", bg="#3b404e", relief=FLAT,
                      command=lambda: register_window(round))
...
def register_window(round):
    ...
    register = Button(myFrame, text="Sign in", fg="white", bg="#3b404e", relief=FLAT,
                      command=lambda: login_window(round))
    register.grid(row=3, column=0, columnspan=2, padx=50)
    register.config(highlightthickness=0)
"--------------------------------------------------------Home Window---------------------------------------------------------------------------"
def home_window(round):
    ...
    addBook = Button(myFrame, text="Add Book", fg="#FFFFFF", bg="#3b404e", relief=GROOVE, borderwidth=2,
                     command=lambda: add_window(round))
    ...
    deleteBook = Button(myFrame, text="Delete Book", fg="#FFFFFF", bg="#3b404e", relief=GROOVE,
                        command=lambda: delete_window(round))
    ...
    viewBook = Button(myFrame, text="View Book", fg="#FFFFFF", bg="#3b404e", relief=GROOVE,
                      command=lambda: view_window(round))
    ...
    issueBook = Button(myFrame, text="Issue Book", fg="#FFFFFF", bg="#3b404e", relief=GROOVE,
                       command=lambda: issue_window(round))
    ...
    returnBook = Button(myFrame, text="Return Book", fg="#FFFFFF", bg="#3b404e", relief=GROOVE,
                        command=lambda: return_window(round))
...
def add_window(round):
    ...
    def add(round):
        ...
        if val == True:
            ...
            label_1.config(
                ...
                relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12), width=35, anchor="center")

        elif val == False:
            error = Label(myFrame, text="Unsuccessful!!", fg="white", relief=RAISED, bg="#3b404e", bd=0,
                          font=("Calibri", 12), width=35, anchor="center", )
    ...
    # Name of book Label
    book_name = Label(myFrame, text="Book Name:", relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12),
                      padx=15)
    book_name.grid(row=0, column=0, padx=15, pady=5)

    # Name Of Author Label
    author_name = Label(myFrame, text="Name of Author:", relief=RAISED, fg="white", bg="#3b404e", bd=0,
                        font=("Calibri", 12), padx=15)
    author_name.grid(row=2, column=0, padx=15, pady=5)

    # Date Label
    date_name = Label(myFrame, text="Date \n (YYYY-MM-DD):", relief=RAISED, fg="white", bg="#3b404e", bd=0,
                      font=("Calibri", 12), padx=15)
    date_name.grid(row=5, column=0, padx=15, pady=5)

    # Price Label
    price_name = Label(myFrame, text="Price:", relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12),
                       padx=15)
    ...
    # Category Label
    category_name = Label(myFrame, text="Category:", relief=RAISED, fg="white", bg="#3b404e", bd=0,
                          font=("Calibri", 12))
    ...
    # Category field
    category_name_Enter = Entry(myFrame, highlightbackground="#3b404e", bg="#3b404e", borderwidth=2, fg="white",
                                border=3)
...
def delete_window(round):
    ...
    sno = Label(myFrame, text="Serial No. :", relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12),
                padx=20, pady=20)
    ...
    name_book = Label(myFrame, text="Name of Book :", relief=RAISED, fg="white", bg="#3b404e", bd=0,
...
def view_window(round):
    ...
    # title
    title = Label(myFrame, text="%-15s%-29s%-20s%-10s" % ('S.No', 'Title', 'Author', 'Status'), relief=RAISED,
                  bg="#3b404e", fg='white', width=45, anchor="center", bd=0, font=("Calibri", 12))
    title.grid(row=0, column=0, columnspan=2, padx=10, pady=8)

    # break line
    line = Label(myFrame, text="----------------------------------------------------------------------------",
                 relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12), width=49, anchor='center')
    ...
    for i in my_list:
        # data label
        data = Label(myFrame, text="%-15s%-35s%-30s" % (i[0], i[1], i[2]) + " " + i[3], bg="#3b404e", fg='white',
                     width=50, anchor='w')
...
def issue_window(round):
    ...
    def issuewindow2(round):
        ...
        if result == True:
            label_1 = Label(myFrame,
                            ...
                            relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12), width=35,
                            anchor="center")
            label_1.grid(row=0, column=0, padx=60, pady=5)
        else:
            label_1 = Label(myFrame, text="\n\nBook is currently unavailable.", relief=RAISED, fg="white", bg="#3b404e",
                            bd=0, font=("Calibri", 12), width=35, anchor="center")
            label_1.grid(row=0, column=0, padx=60, pady=5)

        # next button for going to the next step
        next_button_1 = Button(myFrame, image=round, borderwidth=2, bg="#3b404e", fg="white", relief=GROOVE,
                               command=lambda: home_window(round))
    ...
    # label asking for name of book
    name_student = Label(myFrame, text="Name of Student :", relief=RAISED, fg="white", bg="#3b404e", bd=0,
...
def return_window(round):
    ...
    def return_window2(round):
        ...
        message = Label(myFrame, relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12), width=35,
                        anchor="center")
        if value == True:
            message.config(text="The book with serial number: " + number + "\nhas been returned by " + name + ".")
        elif type(value) != bool:
            message.config(
                text="The book of serial number: " + number + "\nis a late return.\nKindly pay fine of Rs. " + str(
                    value))
        elif value == False:
    ...
    student_name = Label(myFrame, text="Name of student:", relief=RAISED, fg="white", bg="#3b404e", bd=0,
                         font=("Calibri", 12), padx=20, pady=15)
    ...
    student_name_Enter = Entry(myFrame, highlightbackground="#3b404e", bg="#3b404e", borderwidth=2, fg="white",
                               border=3)
...
def modes(round):
    ...
    radio_button_1 = Radiobutton(myFrame, text="defaulter's list", font=("Calibri", 12), width=16, fg="white",
                                 bg="#3b404e", selectcolor="#3b404e", activebackground="#3b404e", anchor="w",
                                 variable=r, value=1, command=lambda: table(round, radio_button_1, radio_button_2))
    radio_button_1.place(relx=0.5, rely=0.3)

    # radio button 2 for book lost
    radio_button_2 = Radiobutton(myFrame, text="book lost", font=("Calibri", 12), width=16, fg="white", bg="#3b404e",
                                 selectcolor="#3b404e", activebackground="#3b404e", anchor="w", variable=r, value=2,
                                 command=lambda: booklost(round, radio_button_1, radio_button_2))
...
def table(round, radio_button_1, radio_button_2):
    ...
    Title = Label(myFrame, text="%-10s%-25s%-15s%-10s" % ('Sr. No.', 'Name of Student', 'Due Date', 'Fine'),
                  relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12))
    Title.grid(row=3, column=0, columnspan=2, padx=15, pady=8)

    line = Label(myFrame, text="----------------------------------------------------------------------------",
                 relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12), width=49, anchor='w')
    ...
    try:
        for i in res:
            def_list = Label(myFrame, text="%-10s%-25s%-15s%-10s" % (i[0], i[1], i[2], i[3]), relief=RAISED, fg="white",
...
def booklost(round, radio_button_1, radio_button_2):
    ...
    name = Label(myFrame, text="Name of Defaulter:", relief=RAISED, fg="white", bg="#3b404e", bd=0,
                 font=("Calibri", 12), padx=40, pady=10)
    ...
    # label asking for name of book
    serial = Label(myFrame, text="Serial number:", relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12),
                   padx=40, pady=10)
    ...
    def disable_2(round):
        ...
        if r_2.get() == 3:
            ...
            label_1 = Label(myFrame, text=name + " is fined for Rs." + str(price) + "\n", relief=RAISED, fg="white",
                            bg="#3b404e", bd=0, font=("Calibri", 12), width=46, anchor='center')
    ...
    option = Label(myFrame, text="Options:", relief=RAISED, fg="white", bg="#3b404e", bd=0, font=("Calibri", 12),
                   width=5, anchor='center')
    option.grid(row=5, column=0, padx=40, pady=5, sticky='news')

    # radio button 3 for pay price
    radio_button_3 = Radiobutton(myFrame, text="pay original price", font=("Calibri", 12), width=22, fg="white",
                                 bg="#3b404e", selectcolor="#3b404e", activebackground="#3b404e", variable=r_2, value=3,
                                 command=lambda: disable_2(round))
    radio_button_3.grid(row=5, column=1, pady=5)

    # radio button 4 for add new book
    radio_button_4 = Radiobutton(myFrame, text="add new book", font=("Calibri", 12), width=20, fg="white", bg="#3b404e",
                                 selectcolor="#3b404e", activebackground="#3b404e", anchor="center", variable=r_2,
                                 value=4, command=lambda: disable_2(round))
...
bg = ImageTk.PhotoImage(Image.open("0_mMD5SlIbFvgkGo3l.jpeg"))
...
button_image = Image.open("Rounded Button.png")
button_image = button_image.resize((110, 40))
