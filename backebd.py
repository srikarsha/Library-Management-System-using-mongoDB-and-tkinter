...
import pymongo
import datetime
#update the connectionString according to your mondoDB
connectionString = "mongodb+srv://<username>:<password>@cluster0.j0ww.mongodb.net/"
myclient = pymongo.MongoClient(connectionString)
database = myclient["LMS"]
...
def login_data(username, password):
    x = database.login.count_documents(
        {"$expr": {"$and": [{"$eq": ["$username", str(username)]}, {"$eq": ["$password", str(password)]}]}})
    return x
    ...
    for i in x:
        ...
        if duedate < today:
            ...
            if flag == True:
                ...
                delta = today - date1
                fine = delta.days * 5
                dict1 = {"serial number": i["serial number"], "book name": bookname,
                         "defaulter name": i["student name"], "due date": i["due date"], "number of days": delta.days,
                         "fine": fine}
